...
def djmodel(Tx, n_a, n_values):
    ...
    for t in range(Tx):
        ...
        x = reshapor(x)                                                           # Reshape((1, n_values))(x)  
        # Step 2.C: Perform one step of the LSTM_cell
        a, _, c = LSTM_cell(x, initial_state=[a, c])                              # LSTM(n_a, return_state = True)(x, initial_state=[a, c])
        # Step 2.D: Apply densor to the hidden state output of LSTM_Cell
        out = densor(a)                                                           # Dense(n_values, activation='softmax')(a)                            
        # Step 2.E: add the output to "outputs"
        outputs.append(out)
        
    # Step 3: Create model instance
    model = Model(inputs=[X, a0, c0], outputs=outputs, name='djmodel')
...
def predict_and_sample(inference_model, x_initializer = x_initializer, a_initializer = a_initializer, 
    ...
    results = to_categorical(indices, 78)
    #print(to_categorical.__doc__)
    #print("results.shape: ", results.shape)
    #print("results ",results[12])
